...
struct _CSSOM_CSSStyleDeclarationValue {
  ...
  CSSOM_FSMIter_CSSPropertyValue lastShorthand;
  ...
  CSSOM_CSSStyleDeclaration *parentStyle)
{
  ...
  it = CSSOM_FSM_CSSPropertyValue_end(fsm);
  ...
  values->fsm = fsm;
  values->lastShorthand = it;
  values->removedPropertyHolder = NULL;

  size = CSSOM__nproperties(cssom);
  for (i = 0; i < size; ++i) {
    ...
    if (property == NULL) {
      CSSOM_CSSStyleDeclarationValue_release(values);
      return NULL;
    }

    it = CSSOM_FSM_CSSPropertyValue_insert(values->fsm,
      CSSOM_CSSPropertyValue__name(property), property);
  ...
  const CSSOM_CSSStyleDeclarationValue *values)
{
  return CSSOM_FSM_CSSPropertyValue_size(values->fsm) -
    CSSOM__nshorthands(values->cssom);
}
  ...
  CSSOM_CSSStyleDeclarationValue *values, unsigned long index)
{
  ...
  index += CSSOM__nshorthands(values->cssom);
...
static void CSSStyleDeclarationValue__fremoveProperty(
  CSSOM_CSSStyleDeclarationValue *values, int hash);
  ...
  CSSOM_CSSStyleDeclarationValue *values, CSSOM_FSMIter_CSSPropertyValue iter)
{
  ...
  if (setting->nsubhashes == 0) {
    ...
    for (i = 0; i < setting->nsubhashes; ++i)
      CSSStyleDeclarationValue__fremoveProperty(values, setting->subhashes[i]);
  }
}
  ...
  const CSSOM_CSSStyleDeclarationValue *values, int hash)
{
  ...
  it = CSSOM_FSM_CSSPropertyValue_ffind(values->fsm, hash);
  return CSSStyleDeclarationValue__getProperty(values, it);
}
  ...
  const CSSOM_CSSStyleDeclarationValue *values, int hash)
{
  ...
  value = CSSOM_CSSStyleDeclarationValue__fgetProperty(values, hash);
  ...
  CSSOM_CSSStyleDeclarationValue *values, int hash)
{
  ...
  match = CSSOM_FSM_CSSPropertyValue_ffind(values->fsm, hash);
  ...
  CSSOM_CSSStyleDeclarationValue *values, int hash, const char *value)
{
  ...
  if (value == NULL) {
    CSSStyleDeclarationValue__fremoveProperty(values, hash);
    return;
  }

  sproperty = CSSOM__propertySetting(values->cssom, hash)->name;
...
CSSOM_CSSStyleDeclarationValueIter CSSOM_CSSStyleDeclarationValue__begin(
  CSSOM_CSSStyleDeclarationValue *values)
{
  return CSSOM_FSMIter_CSSPropertyValue_next(values->lastShorthand);
}
