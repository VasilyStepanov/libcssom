...
static const SAC_LexicalUnit** cssFloat(const SAC_LexicalUnit **begin,
...
const SAC_LexicalUnit** CSSOM_LexicalUnitRange_float(
  ...
  struct _CSSOM_LexicalUnitRange *ranges)
{
  ...
  tail = cssFloat(begin, end);
  if (tail == begin) return begin;

  if (ranges != NULL)
    _CSSOM_SET_RANGE(ranges[0], CSSOM_FLOAT_PROPERTY, begin, tail);
  return tail;
}
